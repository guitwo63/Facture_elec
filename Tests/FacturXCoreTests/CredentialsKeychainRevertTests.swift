...
final class CredentialsKeychainRevertTests: XCTestCase {
    ...
    private func clearAllKeys() {
        for key in allKeys {
            AppPersistence.defaults.removeObject(forKey: env.key(key))
    ...
    func testSMTPSettingsMigratesPasswordFromLegacyKeychainEntry() throws {
        ...
        legacy.password = ""
        AppPersistence.defaults.set(try JSONEncoder().encode(legacy), forKey: env.key("facturx.smtp.credentials.v1"))
    ...
    func testSMTPSettingsNeverRetriesKeychainMigrationOnceFlagIsSet() throws {
        ...
        XCTAssertTrue(AppPersistence.defaults.bool(forKey: env.key("facturx.smtp.keychainCleanupDone.v1")))
    ...
    func testSMTPSettingsSavePersistsPasswordInPlainUserDefaultsAndClearsKeychain() throws {
        ...
        let data = AppPersistence.defaults.data(forKey: env.key("facturx.smtp.credentials.v1"))
    ...
    func testPCloudSettingsMigratesPasswordFromLegacyKeychainEntry() throws {
        ...
        legacy.password = ""
        AppPersistence.defaults.set(try JSONEncoder().encode(legacy), forKey: env.key("facturx.pcloud.credentials.v1"))
    ...
    func testChorusProSettingsMigratesBothSecretsFromLegacyKeychainEntries() throws {
        ...
        legacy.techPassword = ""
        AppPersistence.defaults.set(try JSONEncoder().encode(legacy), forKey: env.key("facturx.choruspro.credentials.v1"))
    ...
    func testSuperPDPSettingsMigratesClientSecretFromLegacyKeychainEntry() throws {
        ...
        legacy.clientSecret = ""
        AppPersistence.defaults.set(try JSONEncoder().encode(legacy), forKey: env.key("facturx.superpdp.credentials.v1"))
    ...
    func testSuperPDPSettingsSavePersistsClientSecretInPlainUserDefaults() throws {
        ...
        let data = AppPersistence.defaults.data(forKey: env.key("facturx.superpdp.credentials.v1"))
