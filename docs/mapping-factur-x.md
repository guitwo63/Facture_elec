...
| `seller.siren` | `ram:SellerTradeParty/ram:SpecifiedLegalOrganization/ram:ID` (schéma 0002) | BT-30 | BR-FR-10 (obligatoire, exactement 9 chiffres), BT-30-LUHN (clé Luhn, contrôle interne), BR-FR-13 (SIREN ou identifiant électronique) | erreur (absent ou mal formé) / avertissement (clé) / erreur (ni l'un ni l'autre) |
| `seller.siret` | — (non émis ; serait le BT-29, schéma 0009) | — | BR-FR-09 (14 chiffres, clé Luhn) | avertissement |
| `seller.endpointID` | `ram:SellerTradeParty/ram:URIUniversalCommunication/ram:URIID` (schéma choisi, BT-34-1 : 0225 annuaire par défaut, 0009 SIRET, 9957 n° TVA français, EM e-mail ; déduit du SIREN en 0225 si vide) | BT-34 | BR-FR-13 (SIREN ou identifiant électronique), BR-FR-23 (caractères admis en schéma 0225), BR-FR-25 (125 caractères au plus, tout schéma), BR-CL-25 (schéma de la liste EAS) | erreur (BR-CL-25 : avertissement sur une facture reçue) |
...
| `buyer.siren` | `ram:BuyerTradeParty/ram:SpecifiedLegalOrganization/ram:ID` (schéma 0002) | BT-47 | BR-FR-32 (exactement 9 chiffres), BR-FR-11 (destinataire établi en France sans SIREN), BT-47-LUHN (clé Luhn, contrôle interne), BR-FR-12 (SIREN ou identifiant électronique) | erreur (mal formé) / avertissement (absent en France, clé) / erreur (ni l'un ni l'autre) |
| `buyer.siret` | — (non émis ; serait le BT-46, schéma 0009) | — | BR-FR-09 (14 chiffres, clé Luhn) | avertissement |
| `buyer.endpointID` | `ram:BuyerTradeParty/ram:URIUniversalCommunication/ram:URIID` (schéma choisi, BT-49-1 : 0225 annuaire par défaut, 0009 SIRET, 9957 n° TVA français, EM e-mail ; déduit du SIREN en 0225 si vide) | BT-49 | BR-FR-12 (SIREN ou identifiant électronique), BR-FR-23 (caractères admis en schéma 0225), BR-FR-25 (125 caractères au plus, tout schéma), BR-CL-25 (schéma de la liste EAS), BR-FR-21 (adresse 0225 qui ne désigne pas le SIREN ; destinataire français à SIREN hors de l'annuaire) | erreur / avertissement (BR-FR-21 hors de l'annuaire ; BR-CL-25 sur une facture reçue) |
...
| `line.unit` | `ram:BilledQuantity/@unitCode` (C62 si vide, sans espaces autour ; voir « Codes d'unité ») | BT-130 | BR-23 ; BR-CL-23 (code hors de la liste UN/ECE Rec 20 + Rec 21 du Schematron) | avertissement ; erreur (avertissement sur une facture reçue) |
...
| BR-CL-23 | unité (BT-130) | erreur / avertissement | Code hors de la liste UN/ECE Rec 20 + Rec 21 du Schematron (2162 codes, `UnitCodeList`), comparé sous sa forme émise ; pour un ancien code, le message donne l'unité à choisir (PCE → « Pièce (H87) »…) ; facture reçue : avertissement |
| BR-CL-25 | schéma de l'identifiant électronique émetteur / destinataire (BT-34-1 / BT-49-1) | erreur / avertissement | Schéma émis hors de la liste EAS du Schematron (102 codes, `EASCodeList`), comparé sous sa forme émise (vide, « FR:SIRENE » et « 0183 » deviennent 0225) ; facture reçue : avertissement |
...
| BR-FR-10 | SIREN émetteur (BT-30) | erreur | Obligatoire, même avec un identifiant électronique (BT-34), et fait d'exactement 9 chiffres sous le schéma 0002, comparé sous sa forme émise (espaces admis seulement autour) ; facture reçue : absence non contrôlée |
| BR-FR-11 | SIREN destinataire (BT-47) | avertissement | Destinataire établi en France sans SIREN : obligatoire pour une entreprise assujettie, sans objet pour un particulier ; le Schematron ne l'applique qu'avec une note BAR = B2B, non émise ; facture émise seulement |
...
| BR-FR-21 | identifiant électronique destinataire (BT-49) | erreur / avertissement | Destinataire établi en France, à SIREN (schéma 0002, 9 chiffres) : erreur si son adresse émise sous le schéma 0225 ne désigne pas ce SIREN (ni « SIREN » ni « SIREN_… », forme d'ARVERNX-SaaS, plus stricte que le `starts-with` du Schematron) ; avertissement si elle n'est pas sous le schéma 0225 de l'annuaire (EM, 0009, 9957…), que la PDP exige entre entreprises françaises assujetties ; le Schematron ne l'applique qu'avec une note BAR = B2B, non émise ; facture émise seulement |
| BR-FR-23 | identifiant électronique émetteur / destinataire (BT-34 / BT-49) | erreur | Sous le schéma 0225, lettres sans accent, chiffres et « - », « _ », « . » seulement (ni espace, ni « @ », ni « : », ni « + »), comparé sous sa forme émise : l'adresse saisie, sinon le SIREN ; un autre schéma n'est pas contrôlé ; facture reçue : contrôlée aussi |
| BR-FR-25 | identifiant électronique émetteur / destinataire (BT-34 / BT-49) | erreur | 125 caractères au plus, quel que soit le schéma, comptés comme `string-length` (en points de code) sous la forme émise : l'adresse saisie, sinon le SIREN ; facture reçue : contrôlée aussi |
| BR-FR-32 | SIREN destinataire (BT-47) | erreur | Exactement 9 chiffres sous le schéma 0002, comparé sous sa forme émise ; un identifiant d'un autre schéma n'est pas contrôlé |
...
| BT-30-LUHN / BT-47-LUHN | SIREN émetteur / destinataire (BT-30 / BT-47) | avertissement | Clé de Luhn fausse sur un SIREN de 9 chiffres, que la PDP accepte (faute de frappe probable) — contrôle interne |
...
- **SIREN** (BT-30, BT-47) : le Schematron France CTC exige le SIREN du vendeur sur toute facture (BR-FR-10, fatal), même quand une adresse électronique satisfait BR-FR-13, et 9 chiffres exactement pour tout identifiant légal de schéma 0002 (BR-FR-10, BR-FR-32 : `matches(normalize-space(.), '^\d{9}$')`, sans clé de Luhn). Le SIREN est émis sans espaces autour (`CIIXMLGenerator.xmlSiren`), la chaîne que contrôlent ces règles ; « 732 829 320 » reste refusé. `SireneValidator.isValidSiren`, qui colore l'icône du champ SIREN et la fiche de l'annuaire, applique ce même format, plus la clé de Luhn. Jusqu'au 2026-09-24, l'app n'en faisait qu'un avertissement, rien quand le SIREN du vendeur manquait, et acceptait un SIREN avec des espaces : la PDP rejetait ces factures. Même règle qu'ARVERNX-SaaS (F.33), sauf l'identifiant de l'avertissement de clé (BT-30-LUHN / BT-47-LUHN ici, BR-FR-10 / BR-FR-32 là-bas). Vérifié le 2026-09-24 contre le XSD et les Schematron EN16931 et France CTC ; `SirenRulesTests` rejoue les quatre assertions en cause sur le XML généré.
- **Adresse électronique** (BT-34, BT-49, `URIUniversalCommunication/URIID`) : le générateur émet l'adresse saisie, sans espaces ni retours à la ligne autour, sous son schéma (vide, « FR:SIRENE » et « 0183 » deviennent 0225), sinon le SIREN sous le schéma 0225 (`CIIXMLGenerator.xmlEndpoint`). Le sélecteur de schéma (`NormRefs.endpointSchemes`) propose les quatre schémas d'ARVERNX-SaaS, mêmes libellés : « Annuaire français — SIREN (0225) », « SIRET (0009) », « N° TVA français (9957) » et « E-mail (EM) », tous dans la liste EAS (BR-CL-25). Jusqu'au 2026-09-24, il proposait « SIRET (0183) », « Code RNA (0193) » et « Numéro TVA (0200) » : en EAS, ce sont l'IDE suisse, l'identifiant UBL.BE et le code d'entité légale lituanien (listes publiques Peppol et de la Commission européenne). 0183 partait en 0225, 0193 et 0200 tels quels : les validateurs officiels n'y voyaient rien, mais l'adresse ne désignait rien dans l'annuaire. Aucune fiche ni aucun document ne les portait (comptage du 2026-09-24) : rien n'a été migré, et « 0183 » reste émis en 0225, pour qu'un document émis garde son XML. Sous le schéma 0225, le Schematron France CTC n'admet que les lettres ASCII, les chiffres et « + », « - », « _ », « . » (BR-FR-23, fatal : `matches($value, '^[A-Za-z0-9+\-_.]+$')`, sans `normalize-space`). Le contrôle BR-FR-23 de l'app porte sur cette même chaîne (`ElectronicAddressValidator`) et refuse aussi le « + », que ni le message du Schematron ni l'Annexe A ne citent, comme ARVERNX-SaaS. Une adresse e-mail, une espace, un « : » ou un accent (par exemple un suffixe « Comptabilité » dans une adresse de routage de la fiche tiers) bloquent donc l'export et le dépôt ; pour une adresse avec « @ », le message propose le schéma « E-mail (EM) » ; dans l'éditeur de facture, le liseré rouge entoure la partie en cause (`InvoiceParty.hasAdmittedElectronicAddress`). Jusqu'au 2026-09-24, l'app ne contrôlait rien, et la PDP rejetait ces factures. Vérifié le 2026-09-24 sur 28 cas contre le XSD et les Schematron EN16931 et France CTC (seul écart, voulu : le « + ») ; `ElectronicAddressRulesTests` rejoue les deux assertions sur le XML généré. Pour un destinataire établi en France, à SIREN, dont l'adresse émise n'est pas dans l'annuaire (EM, 0009, 9957…), l'app avertit (BR-FR-21, comme ARVERNX-SaaS) : la PDP exige le schéma 0225 entre entreprises françaises assujetties, mais un particulier ou un client hors du e-invoicing peut recevoir sa facture par e-mail. Le Schematron n'applique BR-FR-21 qu'avec une note BAR = B2B, que l'app n'émet pas ; `ElectronicAddressSchemeTests` rejoue l'assertion sur le XML généré, note ajoutée, et l'avertissement tombe exactement quand elle échoue (hors écart voulu : destinataire sans SIREN ou étranger). Voir aussi « Contrôles B2B du destinataire » et « Schéma de l'adresse électronique » ci-dessous.
- **Longueur de l'adresse électronique** (BT-34, BT-49) : le Schematron France CTC limite l'adresse de l'émetteur et du destinataire à 125 caractères, quel que soit son schéma (BR-FR-25, fatal : `string-length(.) le 125`). Le contrôle BR-FR-25 de l'app porte sur l'adresse émise (`CIIXMLGenerator.xmlEndpoint` : l'adresse saisie sans espaces autour, sinon le SIREN), comptée comme `string-length` (`ElectronicAddressValidator.xmlLength`) : en points de code, et non en caractères Swift (un « e » suivi d'un accent combinant en fait deux) ni en unités UTF-16 (un émoji n'en fait qu'un) ; un retour chariot suivi d'un saut de ligne, que le lecteur XML réunit, ne compte qu'une fois. Au-delà, l'export et le dépôt sont bloqués, et le liseré rouge entoure la partie en cause. Même règle qu'ARVERNX-SaaS. Jusqu'au 2026-09-24, l'app ne contrôlait rien, et la PDP rejetait ces factures. Vérifié le 2026-09-24 sur 31 cas contre le XSD et les Schematron EN16931, EXTENDED, EXTENDED-CTC-FR et France CTC, sans écart ; `ElectronicAddressLengthRulesTests` rejoue les deux assertions sur le XML généré. Le moteur XQuery de Foundation compte un émoji pour deux, contrairement à Saxon : ce cas est testé à part.
- **Contrôles B2B du destinataire** (BR-FR-11, BR-FR-21 ; décisions de Guillaume du 2026-09-24, mêmes règles qu'ARVERNX-SaaS, `_b2b_buyer_rules`) : le Schematron France CTC ne les applique qu'avec la note BAR = B2B, que l'app n'émet pas ; la plateforme reconnaît d'elle-même une facture entre assujettis français. Sur une facture émise, pour un destinataire établi en France : sans SIREN, avertissement BR-FR-11 (bloquer interdirait toute facture à un particulier) ; à SIREN (schéma 0002, 9 chiffres), une adresse émise sous le schéma 0225 qui ne désigne pas ce SIREN est une erreur BR-FR-21, parce que l'annuaire enverrait la facture à une autre entreprise (adresse restée d'un autre client, SIREN corrigé sans l'adresse). La forme attendue est « SIREN » ou « SIREN_… » (SIRET, suffixe, code de routage : `ElectronicAddressValidator.designatesSiren`, la forme que compose `PartyRoutingAddress.composedAddress`) : plus stricte que le Schematron, qui n'exige que `starts-with($endpointID, $siren)` et laisse passer un SIRET seul. Dans l'éditeur, le liseré rouge entoure le destinataire. Vérifié le 2026-09-24 sur les XML générés, note BAR = B2B ajoutée, avec Saxon et les XSLT officiels ; `FrenchBuyerB2BRulesTests` rejoue les deux assertions (seul écart, voulu : la forme « SIREN_ »). Comptage sur les données réelles (13 factures, 14 fiches) : aucune facture nouvellement bloquée ; deux brouillons déjà bloqués reçoivent l'avertissement BR-FR-11 ; une fiche fournisseur de l'environnement de test porte une adresse 0225 d'un autre SIREN.
- **Schéma de l'adresse électronique** (BT-34-1, BT-49-1, BR-CL-25) : le Schematron EN16931 n'admet que les 102 codes de la liste EAS (liste 17 du codedb EN16931, liste 13 en EXTENDED : « Value of '@schemeID' is not allowed », sans identifiant de règle) ; EXTENDED-CTC-FR le nomme BR-CL-25 (fatal) et admet en plus 0219 et 0220 ; France CTC et le XSD ne le vérifient pas. Le sélecteur ne propose que des codes admis, mais un import CSV de tiers, un XML reçu ou une ancienne fiche peuvent en apporter un autre (« SIRET », « em » en minuscules…), que le générateur écrivait tel quel sans que l'app ne dise rien. Le contrôle porte sur le schéma émis (`EASCodeList`, liste embarquée avec sa recette de régénération, même liste qu'ARVERNX-SaaS) : erreur sur une facture émise, avertissement sur une facture reçue (décision du 2026-09-24, comme BR-CL-23) ; le liseré rouge entoure la partie en cause (`InvoiceParty.hasAdmittedEndpointScheme`). Vérifié le 2026-09-24 contre les Schematron EN16931, EXTENDED et EXTENDED-CTC-FR : l'app bloque exactement ce qu'ils rejettent ; `EASSchemeRuleTests` rejoue l'assertion d'EXTENDED-CTC-FR. Aucune fiche ni facture réelle ne porte de schéma hors liste (comptage du 2026-09-24).
...
- **Codes d'unité** (BT-130) : le Schematron n'admet que les 2162 codes de sa liste (UN/ECE Rec 20 avec l'extension Rec 21 : liste 8 du codedb EN16931, liste 15 en EXTENDED, BR-CL-23 fatal en EXTENDED-CTC-FR) ; le XSD et le Schematron Order-X n'en vérifient aucun. Les 20 unités du sélecteur en font partie (`UnitCodeListTests`, liste embarquée dans `UnitCodeList` avec sa recette de régénération). Jusqu'au 2026-09-24, cinq n'en faisaient pas partie : KTM « Kilomètre », PCE « Pièce », PCK « Paquet », BX « Boîte » et ROL « Rouleau », devenus KMT, H87, XPK, XBX et XRO (`NormRefs.legacyUnitReplacements`). Au chargement, les brouillons de facture, de commande et d'achat saisi à la main passent au nouveau code, comme une copie faite depuis un document émis (dupliquer, avoir, acompte, solde, commande → facture). Un document émis garde son code : BR-CL-23 le signale, et bloque un nouvel export de la facture. Les devis n'ont pas de sélecteur d'unité (C62). Le code est émis sans espaces autour (`CIIXMLGenerator.xmlUnitCode`), la chaîne que contrôle BR-CL-23 ; le sélecteur affiche un code hors liste sous la forme « Autre… (PCE) ». Vérifié le 2026-09-24 sur 31 cas (XSD, Schematron EN16931, EXTENDED, EXTENDED-CTC-FR et France CTC) : l'app bloque exactement ce que ces validateurs rejettent.
