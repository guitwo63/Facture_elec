...
| `seller.endpointID` | `ram:SellerTradeParty/ram:URIUniversalCommunication/ram:URIID` (schéma 0225 ; déduit du SIREN si vide) | BT-34 | BR-FR-13 (SIREN ou identifiant électronique), BR-FR-23 (caractères admis en schéma 0225) | erreur |
...
| `buyer.endpointID` | `ram:BuyerTradeParty/ram:URIUniversalCommunication/ram:URIID` (schéma 0225 ; déduit du SIREN si vide) | BT-49 | BR-FR-12 (SIREN ou identifiant électronique), BR-FR-23 (caractères admis en schéma 0225) | erreur |
...
- **Adresse électronique** (BT-34, BT-49, `URIUniversalCommunication/URIID`) : le générateur émet l'adresse saisie, sans espaces ni retours à la ligne autour, sous son schéma (vide, « FR:SIRENE » et « 0183 » deviennent 0225), sinon le SIREN sous le schéma 0225 (`CIIXMLGenerator.xmlEndpoint`). Sous le schéma 0225, le Schematron France CTC n'admet que les lettres ASCII, les chiffres et « + », « - », « _ », « . » (BR-FR-23, fatal : `matches($value, '^[A-Za-z0-9+\-_.]+$')`, sans `normalize-space`). Le contrôle BR-FR-23 de l'app porte sur cette même chaîne (`ElectronicAddressValidator`) et refuse aussi le « + », que ni le message du Schematron ni l'Annexe A ne citent, comme ARVERNX-SaaS. Une adresse e-mail, une espace, un « : » ou un accent (par exemple un suffixe « Comptabilité » dans une adresse de routage de la fiche tiers) bloquent donc l'export et le dépôt ; dans l'éditeur de facture, le liseré rouge entoure la partie en cause (`InvoiceParty.hasAdmittedElectronicAddress`). Jusqu'au 2026-09-24, l'app ne contrôlait rien, et la PDP rejetait ces factures. Vérifié le 2026-09-24 sur 28 cas contre le XSD et les Schematron EN16931 et France CTC (seul écart, voulu : le « + ») ; `ElectronicAddressRulesTests` rejoue les deux assertions sur le XML généré.
